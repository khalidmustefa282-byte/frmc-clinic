...
const _ROLE_MAP = {
  admin:      {
    ...
    perms: ['dashboard','reception','patients','dental','hemorrhoid','medical',
            'pharmacy','expenses','reports','receipts','management','accounting','settings'],
  },
  reception:  {
    label: 'Reception',
    perms: ['reception','patients','pharmacy'],
  },
  dental:     { label: 'Dental Doctor',  perms: ['dental','patients'] },
  hemorrhoid: { label: 'Hemorrhoid Dr.', perms: ['hemorrhoid','patients'] },
  medical:    { label: 'Medical Doctor', perms: ['medical','patients'] },
  pharmacy:   { label: 'Pharmacist',     perms: ['pharmacy','patients'] },
};
...
async function API_getCurrentUser() {
  try {
...
async function API_loadReceptions(mode) {
  try {
    let qs;
    if (mode === 'all') {
      qs = '?limit=500';
    } else if (mode && /^\d{4}-\d{2}-\d{2}$/.test(mode)) {
      qs = '?date=' + mode + '&limit=500';
    } else {
      const today = new Date().toISOString().slice(0, 10);
      qs = '?date=' + today + '&limit=200';
    }
    const res      = await _apiFetch('/consultations' + qs);
    const rows     = _d(res) || [];
    ...
    return rows.map(function(c) {
      const inv = c.consultationInvoice || c.invoice || null;
      const st  = inv && inv.status;
      const pay = st==='paid'?'paid':st==='partial'?'partial':'pending';
      return {
        id:c.id,_bid:c.id,patientId:c.patient_id,
        invoiceId:(inv && inv.id) || c.invoice_id || null,
        invoiceNoReal:(inv && inv.invoice_number) || null,
